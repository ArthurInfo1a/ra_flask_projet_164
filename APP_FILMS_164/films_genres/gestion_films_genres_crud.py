...
def edit_genre_film_selected():
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_afficher = """SELECT id_personnes, nom FROM t_personnes ORDER BY id_personnes ASC"""
...
def update_genre_film_selected():
    if request.method == "POST":
        try:
            ...
            strsql_delete_genre_film = """DELETE FROM t_personne_avoir_matériel WHERE fk_personnes = %(value_fk_genre)s AND fk_materiel = %(value_fk_film)s"""
